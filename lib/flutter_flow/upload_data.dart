...
import 'package:sosyal_medya/helper/image_service.dart';
...
Future<List<SelectedFile>?> selectMediaWithSourceBottomSheet({
  ...
  String imageType = 'post', // 'profile', 'post', 'story'
}) async {
...
Future<List<SelectedFile>?> selectMedia({
  ...
  if (multiImage) {
    ...
    return Future.wait(pickedMedia.asMap().entries.map((e) async {
      final index = e.key;
      final media = e.value;
      var mediaBytes = await media.readAsBytes();
      
      // Apply compression if enabled and not a video
      if (compressImages && !isVideo) {
        switch (imageType) {
          case 'profile':
            mediaBytes = await ImageService.optimizeProfileImage(mediaBytes);
            break;
          case 'story':
            mediaBytes = await ImageService.optimizeStoryImage(mediaBytes);
            break;
          case 'post':
          default:
            mediaBytes = await ImageService.optimizePostImage(mediaBytes);
            break;
        }
      }
  ...
  final pickedMedia = await pickedMediaFuture;
  var mediaBytes = await pickedMedia?.readAsBytes();
  if (mediaBytes == null) {
    return null;
  }
...
bool validateFileFormat(String filePath, BuildContext context) {
  ...
  ScaffoldMessenger.of(context)
    ...
    ..showSnackBar(SnackBar(
      content: Text('Geçersiz dosya biçimi: ${mime(filePath)}'),
    ));
  return false;
}
