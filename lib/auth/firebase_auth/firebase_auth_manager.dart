...
class FirebasePhoneAuthManager extends ChangeNotifier {
  ...
  void Function(BuildContext) get onCodeSent => _onCodeSent == null ? (_) {} : _onCodeSent!;
...
class FirebaseAuthManager extends AuthManager
  ...
  Future deleteUser(BuildContext context) async {
    try {
      if (!loggedIn) {
        print('Hata: oturum açmış kullanıcı yokken kullanıcı silinmeye çalışıldı!');
        return;
      }
  ...
  Future updateEmail({
    ...
    try {
      if (!loggedIn) {
        print('Hata: oturum açmış kullanıcı yokken e-posta güncellenmeye çalışıldı!');
        return;
      }
  ...
  Future updatePassword({
    ...
    try {
      if (!loggedIn) {
        print('Hata: oturum açmış kullanıcı yokken parola güncellemeye çalışıldı!');
        return;
      }
  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Hata oluştu, lütfen daha sonra tekrar deneyiniz.')),
      );
    ...
    phoneAuthManager.addListener(() {
      ...
      if (phoneAuthManager.triggerOnCodeSent) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Hata oluştu, lütfen daha sonra tekrar deneyiniz.'),
        ));
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : SosyalMedyaFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' => 'Hata: E-posta zaten farklı bir hesap tarafından kullanılıyor',
        'INVALID_LOGIN_CREDENTIALS' =>
          'Hata: Sağlanan kimlik doğrulama bilgisi yanlış, hatalı biçimlendirilmiş veya süresi dolmuş',
        'invalid-credential' =>
          'Hata: E-posta veya şifre yanlış. Lütfen kontrol edip tekrar deneyin.',
        'user-not-found' => 'Hata: Bu e-posta adresiyle kayıtlı kullanıcı bulunamadı.',
        'wrong-password' => 'Hata: Şifre yanlış. Lütfen tekrar deneyin.',
        'user-disabled' => 'Hata: Bu hesap devre dışı bırakılmış.',
        'too-many-requests' =>
          'Hata: Çok fazla başarısız giriş denemesi. Lütfen daha sonra tekrar deneyin.',
        _ => 'Hata oluştu, lütfen bilgilerinizi kontrol ediniz.',
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
