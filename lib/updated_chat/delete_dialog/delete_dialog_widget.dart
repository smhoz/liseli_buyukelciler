...
class _DeleteDialogWidgetState extends State<DeleteDialogWidget>
  ...
  Future<void> groupChatdelete() async {
    final chatQuery = await FirebaseFirestore.instance
        .collection('chats')
        .where('group_chat_id', isEqualTo: widget.chatList!.groupChatId)
        .get();

    if (chatQuery.docs.isNotEmpty) {
      await chatQuery.docs.first.reference.delete();
      print('Döküman başarıyla silindi.');
      Navigator.pop(context);
      Navigator.pop(context);
      CodeNoahDialogs(context).showFlush(
        type: SnackType.success,
        message: 'Sohbet Başarıyla Silinmiştir',
      );
    } else {
      print('Silinecek döküman bulunamadı.');
      print(widget.chatList!.groupChatId.toString());
    }
  }
