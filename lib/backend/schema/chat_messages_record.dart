...
class ChatMessagesRecord extends FirestoreRecord {
  ...
  bool hasEventId() => _eventId != null && _eventId!.isNotEmpty;
  ...
  static ChatMessagesRecord fromSnapshot(DocumentSnapshot snapshot) => ChatMessagesRecord._(
        snapshot.reference,
  ...
  @override
  String toString() => 'ChatMessagesRecord(reference: ${reference.path}, data: $snapshotData)';
  ...
  @override
  bool operator ==(other) =>
      other is ChatMessagesRecord && reference.path.hashCode == other.reference.path.hashCode;
}
...
class ChatMessagesRecordDocumentEquality implements Equality<ChatMessagesRecord> {
  ...
  int hash(ChatMessagesRecord? e) => const ListEquality()
      .hash([e?.user, e?.chat, e?.text, e?.image, e?.timestamp, e?.video, e?.eventId]);
