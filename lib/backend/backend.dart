  ...
  List<StreamSubscription?>? streamSubscriptions,
    ...
    queryCollectionPage(
      ...
      try {
        ...
        print('Controller appendPage error (ignored): $e');
      }
      if (isStream) {
        final streamSubscription = (page.dataStream)?.listen((List<UserPostsRecord> data) {
          data.forEach((item) {
            final itemIndexes =
                controller.itemList!.asMap().map((k, v) => MapEntry(v.reference.id, k));
            final index = itemIndexes[item.reference.id];
            final items = controller.itemList!;
            if (index != null) {
              items.replaceRange(index, index + 1, [item]);
              controller.itemList = {for (var item in items) item.reference: item}.values.toList();
            }
          });
        });
...
Future<FFFirestorePage<UsersRecord>> queryUsersRecordPage({
  ...
  List<StreamSubscription?>? streamSubscriptions,
    ...
    queryCollectionPage(
      ...
      if (isStream) {
        final streamSubscription = (page.dataStream)?.listen((List<UsersRecord> data) {
          try {
            data.forEach((item) {
              final itemList = controller.itemList;
              if (itemList != null) {
                final itemIndexes = itemList.asMap().map((k, v) => MapEntry(v.reference.id, k));
                final index = itemIndexes[item.reference.id];
                if (index != null) {
                  final items = List<UsersRecord>.from(itemList);
                  items.replaceRange(index, index + 1, [item]);
            ...
            print('Stream update error (ignored): $e');
          }
        });
        streamSubscriptions?.add(streamSubscription);
      }
...
Future<FFFirestorePage<PostCommentsRecord>> queryPostCommentsRecordPage({
  ...
  List<StreamSubscription?>? streamSubscriptions,
    ...
    queryCollectionPage(
      ...
      if (isStream) {
        final streamSubscription = (page.dataStream)?.listen((List<PostCommentsRecord> data) {
          data.forEach((item) {
            final itemIndexes =
                controller.itemList!.asMap().map((k, v) => MapEntry(v.reference.id, k));
            final index = itemIndexes[item.reference.id];
...
Future<FFFirestorePage<UserStoriesRecord>> queryUserStoriesRecordPage({
  ...
  List<StreamSubscription?>? streamSubscriptions,
    ...
    queryCollectionPage(
      ...
      if (isStream) {
        final streamSubscription = (page.dataStream)?.listen((List<UserStoriesRecord> data) {
          data.forEach((item) {
            final itemIndexes =
                controller.itemList!.asMap().map((k, v) => MapEntry(v.reference.id, k));
            final index = itemIndexes[item.reference.id];
...
Future<FFFirestorePage<StoryCommentsRecord>> queryStoryCommentsRecordPage({
  ...
  List<StreamSubscription?>? streamSubscriptions,
    ...
    queryCollectionPage(
      ...
      if (isStream) {
        final streamSubscription = (page.dataStream)?.listen((List<StoryCommentsRecord> data) {
          data.forEach((item) {
            final itemIndexes =
                controller.itemList!.asMap().map((k, v) => MapEntry(v.reference.id, k));
            final index = itemIndexes[item.reference.id];
...
Future<FFFirestorePage<DogsRecord>> queryDogsRecordPage({
  ...
  List<StreamSubscription?>? streamSubscriptions,
    ...
    queryCollectionPage(
      ...
      if (isStream) {
        final streamSubscription = (page.dataStream)?.listen((List<DogsRecord> data) {
          data.forEach((item) {
            final itemIndexes =
                controller.itemList!.asMap().map((k, v) => MapEntry(v.reference.id, k));
            final index = itemIndexes[item.reference.id];
...
Future<FFFirestorePage<FriendsRecord>> queryFriendsRecordPage({
  ...
  List<StreamSubscription?>? streamSubscriptions,
    ...
    queryCollectionPage(
      ...
      if (isStream) {
        final streamSubscription = (page.dataStream)?.listen((List<FriendsRecord> data) {
          data.forEach((item) {
            final itemIndexes =
                controller.itemList!.asMap().map((k, v) => MapEntry(v.reference.id, k));
            final index = itemIndexes[item.reference.id];
...
Future<FFFirestorePage<ChatsRecord>> queryChatsRecordPage({
  ...
  List<StreamSubscription?>? streamSubscriptions,
    ...
    queryCollectionPage(
      ...
      if (isStream) {
        final streamSubscription = (page.dataStream)?.listen((List<ChatsRecord> data) {
          data.forEach((item) {
            final itemIndexes =
                controller.itemList!.asMap().map((k, v) => MapEntry(v.reference.id, k));
            final index = itemIndexes[item.reference.id];
...
Future<FFFirestorePage<ChatMessagesRecord>> queryChatMessagesRecordPage({
  ...
  List<StreamSubscription?>? streamSubscriptions,
    ...
    queryCollectionPage(
      ...
      if (isStream) {
        final streamSubscription = (page.dataStream)?.listen((List<ChatMessagesRecord> data) {
          data.forEach((item) {
            final itemIndexes =
                controller.itemList!.asMap().map((k, v) => MapEntry(v.reference.id, k));
            final index = itemIndexes[item.reference.id];
...
// Also updates app_version if it's missing or different
Future maybeCreateUser(User user) async {
  ...
  if (userExists) {
    ...
    // Check if app_version needs to be updated
    final userData = userSnapshot.data() as Map<String, dynamic>?;
    final currentVersion = userData?['app_version'] as String?;
    if (currentVersion == null || currentVersion != appVersion) {
      print('📱 Kullanıcı app_version güncelleniyor: $currentVersion -> $appVersion');
      await userRecord.update({'app_version': appVersion});

      // Refresh current user document
      currentUserDocument = await UsersRecord.getDocumentOnce(userRecord);
    }
    return;
  }
